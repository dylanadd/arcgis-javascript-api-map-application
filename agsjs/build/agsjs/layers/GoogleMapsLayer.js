...
dojo.declare("agsjs.layers.GoogleMapsLayer", esri.layers.Layer, {
    ...
    _setMap: function (e, a) {
        ...
        dojo.style(g, "height", (e.height || a.offsetHeight) + "px");
        this._gmapDiv = g;
        var b = dojo.create("div", {}, e.id);
        b.id = "gmaps_top_" + g.id;
        dojo.style(b, d);
        this._topDiv = b;
        var f = dojo.create("div", {}, e.id);
        f.id = "gmaps_controls_" + g.id;
        dojo.style(f, dojo.mixin(d, {
            top: "5px",
            left: "5px"
        }));
        this._controlDiv = f;
    ...
    _initGMap: function () {
        ...
        if (window.google && google.maps) {
            ...
            if (agsjs.onGMapsApiLoad) {
                ...
                if (h.toLowerCase().indexOf("http") == -1) {
                    h = "http:"
                }
        ...
        if (this._gmapDiv) {
            ...
            if (typeof a.opacity !== "undefined") {
                ...
                if (typeof a.filters !== "undefined") {
                    a.filters.alpha.opacity = Math.floor(100 * b)
                } else {
                    if (typeof a.filter !== "undefined") {
                        a.filter = "alpha(opacity:" + Math.floor(b * 100) + ")"
                    }
                }
            }
        }
    ...
    _checkZoomLevel: function () {
        ...
        if (f != null) {
            var b = f.minZoom;
            var e = f.maxZoom;
            var d = this._map.getLevel();
            if (e !== undefined && d > e) {
                this._map.setLevel(e)
            }
            if (b != undefined && d < b) {
                this._map.setLevel(b)
            }
        }
    },
    _setExtent: function (b) {
        ...
        var c = this._map.getLevel();
        this._gmap.fitBounds(this._esriExtentToLatLngBounds(b.expand(0.5)));
        ...
        if (this._mvHandle) {
            if (!this._gmap) {
                ...
                if (!this._svMoved) {
                    ...
                    if (this._streetView) {
                        var a = dojo.query('.gmnoprint img[src*="cb_scout_sprite"]', this._gmapDiv);
                        if (a.length > 0) {
                            dojo.forEach(a, function (d, c) {
                ...
                if (!this._rotateMoved) {
                    ...
                    if (b.length > 0) {
                        dojo.forEach(b, function (d, c) {
                            dojo.place(d.parentNode.parentNode, this._controlDiv);
                            dojo.style(d, "position", "absolute");
                            dojo.style(d, "left", "20px")
                        }, this);
    ...
    _streetViewVisibilityChangeHandler: function () {
        if (this._streetView) {
            ...
            this._toggleEsriControl(a);
            this.onStreetViewVisibilityChange(a)
        }
    },
    _mapTiltChangeHandler: function () {
        var a = this._gmap.getTilt();
        if (a == 45) {
    ...
    dojo.extend(esri.dijit.BasemapGallery, {
        ...
        _onLoadExt: function () {
            ...
            if (this.toggleReference) {
                dojo.forEach(this.basemaps, function (b) {
                    var c = b.getLayers();
                    if (c.length) {
                        this._processReferenceLayersExt(b)
                    } else {
                        c.then(dojo.hitch(this, this._processReferenceLayersExt, b))
                    }
                }, this)
            }
        },
        _processReferenceLayersExt: function (c) {
            ...
            if (d && this.toggleReference) {
                ...
                c._hasReference = false
            }
            var b = 0;
            dojo.forEach(this.basemaps, function (g) {
        ...
        _onSelectionChangeExt: function () {
            ...
            if (c) {
                if (this._googleLayers.length > 0) {
                    dojo.forEach(this._googleLayers, function (f) {
                        this.map.removeLayer(f)
                    }, this);
                    this._googleLayers.length = 0
                }
                dojo.query("input", this.domNode).forEach(function (f) {
                    f.disabled = true
                });
                var e = c.getLayers();
                dojo.forEach(e, function (f) {
                    if (f.type && f.type.indexOf("GoogleMaps") > -1) {
                        ...
                        if (f.type == "GoogleMapsSatellite") {
                            g = agsjs.layers.GoogleMapsLayer.MAP_TYPE_SATELLITE
                        } else {
                            if (f.type == "GoogleMapsHybrid") {
                                g = agsjs.layers.GoogleMapsLayer.MAP_TYPE_HYBRID
                            }
                        }
        ...
        _checkSelectedNode: function () {
            var c = false;
            var b = this.getSelected().getLayers();
            dojo.query(".esriBasemapGallerySelectedNode", this.domNode).forEach(function (d) {
                c = true;
                dojo.query("input", d).forEach(function (e) {
                    e.disabled = false;
                    dojo.forEach(b, function (f) {
                        if (f.isReference) {
                            f.visibility = e.checked
                        }
                    }, this)
                }, this)
            }, this);
        ...
        _setReferenceVis: function (b) {
            dojo.forEach(this.map.layerIds, function (d) {
                var c = this.map.getLayer(d);
                if (c._basemapGalleryLayerType == "reference") {
                    if (b) {
                        c.show()
                    } else {
                        c.hide()
